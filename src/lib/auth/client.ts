 ...
 *
 * In the Grok live-preview iframe, cookies are partitioned, so a popup sign-in
 * captures a bearer token. Deployed / standalone apps use a normal cookie
 * session and never store a token. To sign out call `signOut()` below, not
 * `authClient.signOut()` — the raw call can leave a preview bearer attached.
 */
 ...
 * not `"false"`. Google still needs GOOGLE_CLIENT_ID / SECRET on the server.
 */
export const authEnabled = import.meta.env.VITE_AUTH_ENABLED !== "false";

export { AUTH_PROVIDERS };

const BEARER_KEY = "vellum.bearer-token";
...
/** Grok live-preview iframe only — partitioned cookies need a popup + bearer. */
...
type PopupMessage = { source: "vellum-auth-popup"; token: string | null; error?: string };
...
function openSignInPopup(providerId: string): Window | null {
  ...
  const name = `vellum-signin-${Date.now()}`;
...
function waitForPopupToken(popup: Window): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || (data.source !== "vellum-auth-popup" && data.source !== "grok-auth-popup")) return;
