...
export function LibraryRail({ onClose }: { onClose?: () => void }) {
    ...
    <div className="flex h-full min-h-0 flex-col bg-bg">
      <div className="flex items-center gap-2 px-4 pt-5 pb-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-subtle">Notes + links</p>
        </div>
        {onClose ? (
          <Button variant="quiet" size="icon-sm" onClick={onClose} aria-label="Close library">
            <X />
          </Button>
        ) : null}
      </div>
        ...
        {adding ? (
          <form onSubmit={onAdd} className="px-2 pb-1">
            <input
              ...
              className="h-9 w-full rounded-sm bg-surface px-2 text-sm text-fg placeholder:text-subtle outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
            />
          </form>
        ) : null}
        {folders.map((folder) => {
          ...
          return (
            <div key={folder.id} className="group relative">
              ...
              <button
                type="button"
                className="absolute top-1/2 right-1 flex size-9 -translate-y-1/2 items-center justify-center rounded-sm text-subtle hover:bg-surface-hover hover:text-fg md:opacity-0 md:group-hover:opacity-100"
